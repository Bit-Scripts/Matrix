...
characters = np.asarray(list(' ú.ù,:öøýü×ÖÅ³·ÈØÙÍÐ±´¶¹º¼Â²ÇËÒÓ¾Ú'))
characters_str = ' ú.ù,:öøýü×ÖÅ³·ÈØÙÍÐ±´¶¹º¼Â²ÇËÒÓ¾Ú'  # Caractères ASCII, du plus foncé au plus clair
#characters_str = ' .:-=+*#%@'
...
# Buffer pour le flux de la caméra virtuelle    
    ...
    while running:
        ...
        for column in drop_columns:
            ...
            if row >= resized_image.shape[0] - 1:
                ...
                resized_image[row][column] = int((200 + resized_image[row][column]) / 2) # Intensité maximale
...
def image_fusion():
    global running, ascii_image_result, characters_str, ascii_image_result
    while running:
        rain_ascii_image_cut_line = ""      
        with ascii_image_lock:
...
# Initialize the image_updated variable
image_updated = False
